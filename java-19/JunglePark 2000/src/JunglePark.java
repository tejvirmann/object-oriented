...
/**
 * The JunglePark class extends the PApplet. 
 * 
 * This class initializes the listGUI array list (in setup()), and the background image. 
 * 
 * Furthermore, this class contains the main method which starts the program by calling PApplet 
 * main() method with the name of the PApplet class to run as parameter. JunglePark
 * then runs its setup() method, which sets up the program specifics, as well as the 
 * 3 buttons. The Class also contains many other callback methods that will be run many times 
...
public class JunglePark extends PApplet {
  ...
  public void setup() {
    ...
    AddAnimalButton deerButton = new AddAnimalButton(Tiger, xTiger, yTiger , this );
	AddAnimalButton tigerButton = new AddAnimalButton(Deer, xDeer, yDeer, this);
	ClearButton clearPark = new ClearButton(xClear, yClear, this);
  ...
  public void clear() {
    for (int i = 0; i < listGUI.size(); i++) {
      if (listGUI.get(i) instanceof Animal) {
        listGUI.remove(i);  //a for loop is run and the element is removed at each index.
        i--;
      }
    }
  }
